...
<script type="text/javascript">
	...
	export default{
		...
		data() {
			return {
				...
				form:{
                    ...
                    intro:'',
                    state:'1',
                    pintuan_image:'https://via.placeholder.com/150',
                },
			}
		},
		created() {
		...
		methods: {
			getData() {
                this.ifload = true;
                this.$post_('promotion/pintuan/get_activity',{},(res)=>{
                    if(res.code==0 && res.data){
                        this.form.id = res.data.id;
                        this.form.title = res.data.title;
                        this.form.intro = res.data.intro;
                        this.form.state = res.data.state;
                        this.form.pintuan_image = res.data.pintuan_image;
                    }
                    this.ifload = false;
                })
            ...
            //上传图片
            showImg(url) {
                this.form.pintuan_image = url;
            },

            //保存数据
            saveData() {
                this.$post_('promotion/pintuan/edit_activity',this.form,(res)=>{
                    if(res.code=='0'){
